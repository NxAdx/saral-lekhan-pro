...
> **Handoff Addendum (v2.19.10 / Selection Mode & Visibility Fixes - 2026-06-27)**:
> - **Selection State Mapping**: Mapped `isSelected` and `isSelectionMode` directly inside computed `dataToRender` item objects to force `FlashList` diffing and redrawing. Removed stale callback closures inside recycled list cells.
> - **Contrast Contrast Polish**: Changed `BentoCard` unselected indicator borders from `strokeDim` to `stroke` to ensure they are visible in dark themes.
> - ** Handover details**: Full commit context and next-agent troubleshooting guide are documented in `docs/PRODUCTION_HANDOVER_2026-06-27.md`.
> - **Version State**: Mainline is at **v2.19.10** (versionCode 111).
