...
48) Duplicate version key in package.json blocking OTA updater (resolved 2026-06-26 in v2.19.5)
- Symptom:
  1. The compiled Android app shows version `2.19.1` despite code updates claiming to bump the version to `2.19.4`.
  2. The in-app OTA updater rejects the update flow, showing `current=2.19.1 vs latest=2.17.56 → hasUpdate=false`.
- Cause:
  1. A duplicate `"version": "2.19.1"` key existed at the very bottom of `package.json`.
  2. Node's `require("./package.json")` in `app.config.js` and Groovy's `JsonSlurper` parsing in `android/app/build.gradle` both resolved the last duplicate key, locking the compiled version name statically to `2.19.1`.
- Resolution:
  1. Removed the duplicate `"version"` key from the bottom of `package.json`.
