...
- **FOSS Conversion (Google Decoupling)**: 
  - Eradicated all tracking dependencies (`@react-native-google-signin/google-signin`).
  - Removed Google Drive Sync and proprietary Firebase/OAuth CI checks.
  - Set `updaterMode: "fdroid"` in `app.json` for F-Droid compliance.
- **Local-First SQLite Vault**:
  - Replaced cloud sync with secure offline import/export to the device's downloads folder via `expo-sharing` and `expo-document-picker`.
- **Spark AI Discoverability Upgrade**:
  - Moved Spark AI entrypoints from a hidden bottom menu to an explicit button (`"✨ Spark AI"`) in the Editor `RichToolbar`.
  - Upgraded Settings API Key input with explicit "Save" & "Remove" actions to replace confusing auto-save behavior.
- **Font Resizing UX Reboot**:
  - Ripped out the floating visual popup modal from the Editor.
  - Re-implemented font scaling natively as a visual M3 slider within Settings, instantly persisting `fontSize` across the app.
- **Bug Reporting & Community Connections**:
  - Added dedicated block in Settings to report issues directly to GitHub.
  - Integrated donation mechanisms (UPI, Ko-fi) serving as the current "Freemium" model replacement.

## Next Actions (Post-v3.0 Roadmap)
1. **Prepare F-Droid Store Listing Assets**: Screenshots, description, and source repository tags for the FOSS launch.
2. **Monitor F-Droid Telemetry/Crash Reports**: Wait for feedback on local-first database imports on various OEM storage frameworks.
3. **Typography Polish Watch**: Monitor feedback on Devanagari font rendering (specifically `Hind` and `Poppins` interaction).
4. **Community Translation Setup**: Connect Weblate/Crowdin to open-source the `src/i18n/locales/*.json` files for community improvement.
